...
keys = [
    ...
    Key([mod, "shift"], "r", lazy.spawn(f"{SCRIPTS_DIR}/show-on-rofi.sh"),
        desc="Show my scripts on rofi"),
    Key([mod], "q", lazy.spawn("rofi -show drun"), desc="Show applications"),
    Key([], "Print", lazy.spawn(f"{SCRIPTS_DIR}/common/screenshot.sh"), desc="Screenshot"),
    # Volume up
    Key([], "XF86AudioRaiseVolume", lazy.spawn("amixer -q set Master 1%+"), desc="Master audio +1%"),
    # Volume down
    Key([], "XF86AudioLowerVolume", lazy.spawn("amixer -q set Master 1%-"), desc="Master audio -1%"),
    # Mute/unmute
    Key([], "XF86AudioMute", lazy.spawn("amixer -q set Master toggle"), desc="Toggle master audio"),
